import { AppButton, Heading } from "@/components/ui";
import useProductStore, { useGetCartItemsId } from "@/stores/product.store";
import { Else, If } from "classic-react-components";
...
export default function Cart() {
    const itemsIdList = useGetCartItemsId()
    const { updateCartItems, cartItems, count, decrement, increment, itemsId, removeItem } = useProductStore()
    const total = useMemo(() => {
        if (cartItems.length > 0) {
            return cartItems.reduce((totalPrice, item) => {
                totalPrice += Number(item.price) * itemsId.get(item.id)!
                return totalPrice
            }, 0)
        }
        return 0
    ...
    useEffect(() => {
        async function fetchCartItems() {
            try {
                const promises = itemsIdList.reduce((acc, id) => {
                    acc.push(fetch(`https://dummyjson.com/products/${id}`))
                    return acc
                }, [] as Promise<Response>[])
            ...
            <div className='px-4 lg:px-16 w-full'>
                <If condition={cartItems.length > 0}>
                    <>
                        ...
                        <div className="grid lg:grid-cols-9 gap-4 mt-6">
                            <div className="lg:col-span-5 border border-[#0000001A] rounded-lg px-4 divide-y-2">
                                {
                                    cartItems.map((item, idx) => {
                                        return (
                                            <div key={item.id} className='flex gap-4 py-4 relative'>
                                                <img src={item.images[0]} alt="" width='124px' height='124px' />
                                                <div className='flex flex-col justify-between w-full'>
                                                    <div>
                                                        <h4 className='font-bold text-4 lg:text-[20px] leading-[27px]'>{item.title}</h4>
                                                        <span className='text-[14px] text-[#000000]'>Size: <span className='text-[14px] text-[#00000099]'>Large</span></span>
                                                    </div>
                                                    <div className="flex justify-between">
                                                        <p className="font-bold text-[24px] leading-[32px]">${item.price}</p>
                                                        <div className="flex rounded-full bg-[#F0F0F0]">
                                                            <button className='font-medium text-[16px] px-4 lg:px-[16px]' onClick={() => decrement(item.id)}>-</button>
                                                            <button className='font-medium text-[16px] px-3 '>{itemsId.get(item.id)}</button>
                                                            <button className='font-medium text-[16px] px-4 lg:px-[16px] ' onClick={() => increment(item.id)}>+</button>
                                                        </div>
                                                    </div>
                                                </div>
                                                <button className='absolute right-0 top-4' onClick={() => removeItem(item.id)}>
                                ...
                                <div className='py-4'>
                                    ...
                                    <div className="flex flex-col gap-4">
                                        <div className="flex justify-between">
                                            ...
                                            <span className='font-bold text-[20px] leading-[27px]'>${total}</span>
                                        </div>
                                        <div className="flex justify-between">
                                            <span className='text-[#00000099]'>Discount (-20%)</span>
                                            <span className='font-bold text-[20px] leading-[27px] text-[#FF3333]'>${discount}</span>
                                        </div>
                                        <div className="flex justify-between">
                                ...
                                <div className='py-4'>
                                    <div className="flex flex-col gap-4">
                                        <div className="flex justify-between">
                                            ...
                                            <span className='font-bold text-[24px] leading-[32px]'>${total - discount}</span>
                                        </div>
                                        <div className="flex justify-between gap-4 max-md:flex-wrap">
                                            <input className="flex-1 rounded-full bg-[#F0F0F0] h-12 px-4" placeholder="Add promo code" />
                                            <AppButton className='h-12 bg-black rounded-full text-white px-8 max-md:w-full'>Apply</AppButton>
                                        </div>
                                        <AppButton className='h-12 bg-black rounded-full text-white px-8 flex items-center justify-center gap-3'>
                                            Go to Checkout <span><svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
